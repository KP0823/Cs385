...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while(x!=nullptr){
        	y=x;
        	if (key == x->key()){
        	   cerr << "Warning: Attempt to insert duplicate key '" << key << "'." << std::endl;
        	   delete(z);
        	   return;
        	}
        	if(key<x->key()){
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
    	//goes to each node and deletes it
    	if(n!=nullptr){
    		delete_tree(n->left);
    		delete_tree(n->right);
    		delete n;
    ...
    size_t leaf_count(Node<K, V> *node) const {
        // counts the amount of no nodes with no children
    	if(node==nullptr){
    		return 0;
    	}
    	else{
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        // should check the amount of nodes with at least 1 child
    	if(node==nullptr){
    		return 0;
    	}
    	else{
    ...
    int diameter(Node<K, V> *node) const {
        //checks the the height of the left and right
    	if(node == nullptr){
    		return 0;
    	}
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        //checks when we have reached level, and adds one.
    	if(node==nullptr){
    		return 0;
    	}
    	if(level==0){
    ...
    size_t null_count(Node<K, V> *node) const {
    	// if there is a null we add one else keep checking
        if(node==nullptr){
        	return 1;
        }
        else{
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        // goes to each level and each node and adds the what level they are at
    	if(node!=nullptr){
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
    	//goes to each level of the tree and gets the sum of each level
		if (node != nullptr) {
